...
def detailed_ftp_charts(incident_type, subset, charts_by_type):
    ftp_df = subset.copy()

    # --- Chart 1: Top ftp Senders (Scrollable View) ---
    if "Sender" in ftp_df.columns:
        top_senders = ftp_df["Sender"].value_counts().reset_index()
    ...
    if "Sender" in ftp_df.columns and "Incident Match Count" in ftp_df.columns:
        sender_incident_counts = (
            ftp_df.groupby("Sender")["Incident Match Count"]
            .sum()
            .reset_index()
    ...
    if "Business Unit" in ftp_df.columns and "Sender" in ftp_df.columns:
        unique_senders_df = (
            ftp_df.groupby("Business Unit")["Sender"]
            .nunique()
            .reset_index()
    ...
    if "Business Unit" in ftp_df.columns and "Incident Match Count" in ftp_df.columns:
        incident_counts_df = (
            ftp_df.groupby("Business Unit")["Incident Match Count"]
            .sum()
            .reset_index()
        ...
        # ----- Chart 8: Policy Pie -----
        if "Policy" in ftp_df.columns:
            policy_counts = ftp_df["Policy"].value_counts().reset_index()
            ...
            fig_policy.update_layout(
                title="Policies Triggered for Ftp",
