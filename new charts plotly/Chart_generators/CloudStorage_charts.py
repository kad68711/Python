...
def detailed_cloudstorage_charts(incident_type, subset, charts_by_type):
    cloudStorage_df = subset.copy()

    # --- Chart 1: Top cloudStorage Senders (Scrollable View) ---
    if "Sender" in cloudStorage_df.columns:
        top_senders = cloudStorage_df["Sender"].value_counts().reset_index()
        top_senders.columns = ["Sender", "Count"]

        # ✅ Limit to top 50 senders
        top_senders = top_senders.head(50)

        fig_top_senders = px.bar(
            top_senders,
            x="Sender",
            y="Count",
            title="CloudStorage Senders",
            text="Count",
            color="Sender"
        )
    ...
    if "Sender" in cloudStorage_df.columns and "Incident Match Count" in cloudStorage_df.columns:
        sender_incident_counts = (
            cloudStorage_df.groupby("Sender")["Incident Match Count"]
            .sum()
            .reset_index()
            .sort_values("Incident Match Count", ascending=False)
            .head(50)  # ✅ Limit to top 50 senders
        )
    ...
    # --- Chart 3: Unique Senders per Business Unit (Scrollable View) ---
    if "Business Unit" in cloudStorage_df.columns and "Sender" in cloudStorage_df.columns:
        unique_senders_df = (
            cloudStorage_df.groupby("Business Unit")["Sender"]
            .nunique()
            .reset_index()
            .rename(columns={"Sender": "Unique Senders"})
            .sort_values(by="Unique Senders", ascending=False)
            .head(50)  # ✅ Limit to top 50 business units
        )
        ...
        addlayout(fig_unique_senders,'stack')

        charts_by_type[incident_type].append(create_chart_div(fig_unique_senders))

    # --- Chart 4: Business Units vs Incident Match Counts (Scrollable View) ---
    if "Business Unit" in cloudStorage_df.columns and "Incident Match Count" in cloudStorage_df.columns:
        incident_counts_df = (
            cloudStorage_df.groupby("Business Unit")["Incident Match Count"]
            .sum()
            .reset_index()
        ...
        # --- Chart 5: Policy Pie ---
        if "Policy" in cloudStorage_df.columns:
            policy_counts = cloudStorage_df["Policy"].value_counts().reset_index()
            ...
            fig_policy.update_layout(
                title="Policies Triggered for CloudStorage",
