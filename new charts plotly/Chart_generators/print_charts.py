...
def detailed_print_charts(incident_type, subset, charts_by_type):
    print_df = subset.copy()

    # --- Chart 1: Top Print Senders (Scrollable View) ---
    if "Sender" in print_df.columns:
        top_senders = (
            print_df["Sender"].value_counts()
            .reset_index()
    ...
    if "Sender" in print_df.columns and "Incident Match Count" in print_df.columns:
        sender_incident_counts = (
            print_df.groupby("Sender")["Incident Match Count"]
            .sum()
            .reset_index()
    ...
    if "Business Unit" in print_df.columns and "Sender" in print_df.columns:
        unique_senders_df = (
            print_df.groupby("Business Unit")["Sender"]
            .nunique()
            .reset_index()
    ...
    if "Business Unit" in print_df.columns and "Incident Match Count" in print_df.columns:
        incident_counts_df = (
            print_df.groupby("Business Unit")["Incident Match Count"]
            .sum()
            .reset_index()
        ...
        # ----- Chart 8: Policy Pie -----
        if "Policy" in print_df.columns:
            policy_counts = print_df["Policy"].value_counts().reset_index()
            ...
            fig_policy.update_layout(
                title="Policies Triggered for Print",
