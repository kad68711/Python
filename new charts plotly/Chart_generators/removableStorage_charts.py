...
def detailed_usb_charts(incident_type, subset, charts_by_type):
    usb_df = subset.copy()

    # --- Chart 1: Top USB Senders (Scrollable View) ---
    if "Sender" in usb_df.columns:
        top_senders = (
            usb_df["Sender"].value_counts()
            .reset_index()
    ...
    if "Sender" in usb_df.columns and "Incident Match Count" in usb_df.columns:
        sender_incident_counts = (
            usb_df.groupby("Sender")["Incident Match Count"]
            .sum()
            .reset_index()
    ...
    if "Business Unit" in usb_df.columns and "Sender" in usb_df.columns:
        unique_senders_df = (
            usb_df.groupby("Business Unit")["Sender"]
            .nunique()
            .reset_index()
    ...
    if "Business Unit" in usb_df.columns and "Incident Match Count" in usb_df.columns:
        incident_counts_df = (
            usb_df.groupby("Business Unit")["Incident Match Count"]
            .sum()
            .reset_index()
        ...
        # ----- Chart 8: Policy Pie -----
        if "Policy" in usb_df.columns:
            policy_counts = usb_df["Policy"].value_counts().reset_index()
            ...
            fig_policy.update_layout(
                title="Policies Triggered for USB",
