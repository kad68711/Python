...
df = pd.read_excel("exempted_users_data.xlsx")

def ExemptedUsers_pass_data_charts():
    

    # Get counts for Sender and Recipient Exceptions 
    sender_exceptions_count = df["Sender Exceptions"].nunique()
    recipient_exceptions_count = df["Recipient Exceptions"].nunique()
    

    # Store for rendering in HTML
    exception_summary = {
        "sender": sender_exceptions_count,
        "recipient": recipient_exceptions_count
    }
...
routed, this includes http/https and email data.</li>
    ...
    """,
    """<ul>      
      <li>overall policies triggered</li>
      <li>PII policy (2) triggers for Chassis number,
Voter ID, Mobile number, Driving license,
Email address. (This triggers for atleast 50
matches.)</li>
      <li>PII policy triggers for India Aadhaar Card Number, India Permanent Account Number,
Credit Card Numbers, Credit Card Magnetic Stripe Data. (Note- This triggers for atleast 50
matches.)</li>
      <li>Renewal (2) triggers for Policy number and maximus policy number. (Note- This triggers for
atleast 50 matches found.)</li>
<li>Encrypted data policy triggers for even a single encrypted document</li>
<li>Health renewal policy triggers for attachment above 1</li>
    </ul>
                         """]
    ...
    """
    <ul>
      <li>Top 50 senders successfully sending data outside by making use of exemption given to them</li>
...
with a domain may differ from the total usage count displayed.</li>
    ...
    """
    <ul>
      <li>Shows Top 5 Business Units  communicating with each domain</li>
    ...
    <ul>
      ...
      <li>This count doesn’t match the original Exceptions list which is much higher, but only the
users who actually made use of the exceptions given to them.</li>
    ...
    return generic_block_charts_withnotes,charts_by_type_withnotes,exception_summary
